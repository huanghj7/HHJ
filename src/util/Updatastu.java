...
public void actionPerformed(ActionEvent e){
	...
	if(obj==查找){if(客户编号1.getText().equals(""))JOptionPane.showMessageDialog(this,"请填写查询的客户编号！" );
	    ...
	    else{JOptionPane.showMessageDialog(this,"没有这个编号的客户" );}
	    stmt.close();
	    rs1.close();
	...
	if(obj==修改){if(save==null){JOptionPane.showMessageDialog(this,"还没查找需要修改的客户" );}
	else{
		if(客户编号.getText().equals("")||姓名.getText().equals("")||电话号码.getText().equals("")||联系人姓名.getText().equals("")||联系人电话.getText().equals("")){
			JOptionPane.showMessageDialog(this,"必填信息填满才能修改！" );
              ...
              sql="update Client set clientNo='"+客户编号.getText()+"',clientName='"+姓名.getText()+"',clientTelNo='"+电话号码.getText()+"',clientStreet='"+街道.getText()+"',clientCity='"+城市.getText()+"',clientState='"+所在州.getText()+"',clientZipCode='"+邮政编码.getText()+"',contactName='"+联系人姓名.getText()+"',contactTelNo='"+联系人电话.getText()+"',contactEmailNo='"+联系人邮箱.getText()+"' where  clientNo='"+save+"'";
		      if(save.trim().equals(客户编号.getText().trim())){
		  try{
		    Connection dbConn1=CONN();
			stmt=(Statement)dbConn1.createStatement();
